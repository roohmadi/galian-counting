...
class App:
    ...
    def detect_muatanNew(self, results, current_frame_small,w):
        global skip_double,sabes_count, batu_count, cntFileSaveBatu, cntFileSaveSabes, cntFlag
        global chtruk, tempCy, arah
        global skip_double0, chtruk0
        global captureOK
        ...
        for *xyxy, conf, cls in results.xyxy[0]:
            ...
            if (cy >= YlineDetect1) and (skip_double == 1):
                skip_double = 0
            if (cy >= YlineDetect0) and (cy <= YlineDetect1) and (skip_double == 0):

                for *xyxy, conf, cls in results.xyxy[0]:
                    cxx, cyy = (int(xyxy[0]+(xyxy[2]-xyxy[0])/2),
                          int(xyxy[1]+(xyxy[3]-xyxy[1])/2))

                    label = f'{model.names[int(cls)]} {conf:.2f} {cls}'
                    print(label)
                    if conf>0.65:
                        print("cyy: " + str(cyy) + "   cyTemp: " + str(tempCy) + " arah: " +str(arah) + " chtruk: " +str(chtruk) + " cap: " +str(captureOK))

                        if (cyy >= (YlineDetect0)) and (int(cls) == 0) and (skip_double == 0):
                            chtruk  = 1
                        if (cyy < tempCy) and (cyy >= YlineDetect0):
                            arah = 0
                            tempCy = cyy
                        elif (cyy > tempCy) and (chtruk == 1) and (cyy >= YlineDetect0):
                            tempCy = cyy
                            arah = 1
                        if (chtruk == 1) and (arah == 1):
                            captureOK = 1
                        if (cyy >= (YlineDetect1)) and ((int(cls) == 1) or (int(cls) == 2) or (int(cls) == 0)):
                            skip_double = 0
                            chtruk  = 0
                            tempCy = 0

                        if (int(cls) == 0) and (captureOK == 1):
                            cv2.rectangle(current_frame_small, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), (255,0,0), 2)
                            cv2.putText(current_frame_small, label, (int(xyxy[0]), int(xyxy[1])-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0), 2)
                            cv2.line(current_frame_small, (0, cy),(w, cy), (255, 0,0), thickness=3)
                        if (int(cls) == 1) and (captureOK == 1):
                            skip_double = 1
                            cv2.rectangle(current_frame_small, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), (0,0,255), 2)
                            cv2.putText(current_frame_small, label, (int(xyxy[0]), int(xyxy[1])-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 2)
                            cv2.line(current_frame_small, (0, cy),(w, cy), (0,0,255), thickness=3)
    ...
    def detect_muatan000(self, results, current_frame_small,img_ori,w):
        global skip_double,sabes_count, batu_count, cntFileSaveBatu, cntFileSaveSabes, cntFlag
        ...
        for *xyxy, conf, cls in results.xyxy[0]:
            ...
            if conf>0.65:
                ...
                if (chtruk == 1) and (arah == 1) and (skip_double == 0):
                    captureOK = 1
                ...
                if (cyTruk > Y0) and (int(cls) == 1) and (skip_double == 0) and (chtruk == 1):
                    skip_double = 1
                    cv2.rectangle(img_ori, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), (0,0,255), 2)
...
#-----
    ...
    def detect_muatanOK1(self, results, current_frame_small,img_ori,w,h):
        global skip_double,sabes_count, batu_count, cntFileSaveBatu, cntFileSaveSabes, cntFlag
        global chtruk, tempCy, arah,cyTruk
        global skip_double0, chtruk0
        global captureOK
        ...
        for *xyxy, conf, cls in results.xyxy[0]:
            ...
            if conf>0.65:
                ...
                if diff<0:
                    ...
                    arah=0
                if (cy > (Y0)) and (int(cls) == 0) and (skip_double0 == 0):
                    chtruk  = 1
                if cy > (Y0) and (skip_double == 0) and (chtruk == 1) and (arah == 1) :
                    if cls==1:
                        ...
                        cv2.rectangle(current_frame_small, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), (255,255,0), 2)
                        cv2.putText(current_frame_small, label, (int(xyxy[0]), int(xyxy[1])-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,0), 2)
                if (cy >= (Y1)) and ((int(cls) == 1) or (int(cls) == 2)):
                    skip_double = 0
                    chtruk  = 0
                    tempCy = 0
    ...
    def detect_Obj(self, results, current_frame_small,img_ori,w,h):
        global skip_double,sabes_count, batu_count, cntFileSaveBatu, cntFileSaveSabes, cntFlag
        global chtruk, tempCy, arah,cyTruk
        global skip_double0, chtruk0
        global captureOK
        ...
        for *xyxy, conf, cls in results.xyxy[0]:
            ...
            if conf>0.6:
                ...
                print((cy > (Y1+10)))
                print("")
                if (cy < (Y0-10)) or (cy > (Y0+10)) or (cy < (Y1-10)) or (cy > (Y1+10)):
                    print("cy: " + str(cy) + " Y0: " +str(Y0) + " Y1: " +str(Y1) + " YlineDetect1: " +str(YlineDetect1) )
                    current_time = datetime.datetime.now()
                    tgl = str(current_time.year) + "-" + str(current_time.month) + "-" + str(current_time.day)
                    jam = str(current_time.hour) + ":" + str(current_time.minute) + ":" + str(current_time.second)
                    filenameSave = "Img_" + str(current_time.year) + "_" + str(current_time.month) + "_" + str(current_time.day) +"_" + str(current_time.hour) + "_" + str(current_time.minute) + "_" + str(current_time.second) + "_0S.jpg"
            ...
            #cv2.line(img_ori, (0, Y0),(w, Y0), (255,255, 255), thickness=3)
           # cv2.line(img_ori, (0, Y1),(w, Y1), (255,255, 255), thickness=3)
            #cv2.line(img_ori, (0, YlineDetect1),(w, YlineDetect1), (255,255, 255), thickness=3)
    ...
    def upload_vid_func(self):
        ...
        def browse_file():
            def run_yolov5_on_video():
                ...
                def show_frame():
                    ...
                    if (cntdot % 10) == 0  :
                        ...
                        #cv2.line(current_frame_small, (0, YlineDetect0),(w, YlineDetect0), (255,255, 255), thickness=3)

                        #======

                        #self.detect_muatan(results, crop_img,current_frame_small,w,h)
                        self.detect_Obj(results, crop_img,current_frame_small,w,h)
            ...
            #file_path = 'C:\\Users\\roohm\\vehicle-counting-yolov5\\data\\video\\New\\muatan_double1.mp4'
