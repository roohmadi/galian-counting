...
global skip_double,sabes_count, batu_count, cntFileSaveSabes, cntFileSaveBatu, PINTU, img_del_date
skip_double = 0
sabes_count =  0
batu_count = 0
cntFileSaveSabes = 0
    ...
    for fileN in os.listdir(path_img):
        ...
        if (cek_img_del > 0) and (imgUPL == 'upl'):
            ...
            if get_diff_days > img_del_date:
                ...
                isExistdelUPL = os.path.exists(path_img + fileN)
                #print(isExistdelUPL)
                if isExistdelUPL:
...
#btn_rdcsv = Button(fr_button, text="Webcam", command=reUPLOAD_img)
