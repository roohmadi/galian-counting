...
class CCTVStream :
    ...
    def detect_muatan(self, results, current_frame_small,img_ori,w,h):
        ...
        for *xyxy, conf, cls in results.xyxy[0]:
            ...
            if conf>0.65:
                if (cyTruk > (180)) and (cyTruk < 250) and (int(cls) == 0) and (skip_double0 == 0):
                    chtruk  = 1
                
                if cyTruk > (180) and (skip_double == 0) and (chtruk == 1) and (arah == 1) :
                    current_time = datetime.datetime.now()
                    tgl = str(current_time.year) + "-" + str(current_time.month) + "-" + str(current_time.day)
                    jam = str(current_time.hour) + ":" + str(current_time.minute) + ":" + str(current_time.second)
                    filenameSave = "Img_" + str(current_time.year) + "_" + str(current_time.month) + "_" + str(current_time.day) +"_" + str(current_time.hour) + "_" + str(current_time.minute) + "_" + str(current_time.second) + "_"+ str(int(cls))+"X.jpg"
                    ...
                    skip_double = 1
                if (cyTruk >= (250)) and ((int(cls) == 0) or(int(cls) == 1) or (int(cls) == 2)):
                    skip_double = 0
                    chtruk  = 0
                    tempCy = 0
        if (arah == 1):
            cv2.circle(img_ori, (30,h//2), 10, (0,255,0), 20)
        if (arah == 0):
            cv2.circle(img_ori, (30,h//2), 10, (0,0,255), 20)
