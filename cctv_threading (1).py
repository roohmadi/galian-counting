        ...
        if cek_img_off > 0:
            for fileN in os.listdir(path_img):
                ...
                if (aa[0] == "Img"):
                    print("cocok")
                    filenameSave = fileN
                    filegambarstart = "res_preCap_" + fileN
                    #print(filenameSave) 
                    #print(filegambarstart)
                    isExistCapImg = os.path.exists(path_img + "/" + filenameSave)
                    isExistPreImg = os.path.exists(path_img + "/" +  filegambarstart)
                    if (isExistPreImg and isExistCapImg):
                        ...
                        self.UploadIMGtoPedati(filenameSave, filename,filegambarstart,filenamestart, fileN[-6], host)
                        print("Upload offline suksess...")
        
    def reUPLOAD_img_old(self):
        #print(path_img)
            ...
            if conf>0.5:
                ...
                if imageVal:
                    ...
                    if((int(cls) == 1) or (int(cls) == 2)) and (arah == 1) and (chtruk == 1) and (skip_double0 == 0)  and  (cy > YlineDetect0):
                        ...
                        cv2.imwrite("preCapTemp.jpg", self.img_resizePrecap)
                        print("preCapTemp...")
