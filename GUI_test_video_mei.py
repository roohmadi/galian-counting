...
global skip_double,sabes_count, batu_count, cntFileSaveSabes, cntFileSaveBatu, PINTU, img_del_date, host, weight_file, OStype, start, end
...
def detect_muatan11(results, current_frame_small,img_ori,img_crop_raw,w,h):
        ...
        for *xyxy, conf, cls in results.xyxy[0]:
            ...
            if (int(cls)==0):
                cyTruk = cy
            label = f'{model.names[int(cls)]} {conf:.2f} {cls}'
            #print("cyTruk: " + str(cyTruk) + "  last: " + str(tempCy))

            #---cek cy
            if ( cy not in tracker1) and (int(cls)==0):
                print("cy in list: " + str(cy))
                tracker1.append(cy)
                print(tracker1)
            ...
            if (int(cls)==0):
                ...
                tempCy = cyTruk
                #if (cyTruk == tempCy):
                cv2.circle(img_ori, (cx,cy), 1, (0,255,0), 20)
                #if (cyTruk > tempCy):
                ...
                isExistLastImg = os.path.exists('test_mse.jpg')
                if isExistLastImg:
                    os.remove('test_mse.jpg')
                    cv2.imwrite('test_mse.jpg', img_crop_raw)

def detect_muatanLast(results, current_frame_small,img_ori,img_crop_raw,w,h):
        global skip_double,sabes_count, batu_count, cntFileSaveBatu, cntFileSaveSabes, cntFlag, skipcrop
        global chtruk, tempCy, arah,cyTruk
        global skip_double0, chtruk0
        global captureOK
        ...
        for *xyxy, conf, cls in results.xyxy[0]:
            ...
            if (int(cls)==0):
                cv2.circle(img_ori, (cx,cyTruk), 1, (255,255,255), 20)
                cv2.putText(img_ori, str(cy), (cx+100,cy), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 2)
            ...
            #print(" ")
            #print(" ")
            if (cyTruk > 230) and (cyTruk <= YlineDetect1) and (skipcrop == 0) and (int(cls) == 1 or int(cls) == 2 or int(cls) == 4) and (chtruk == 1) and (arah == 1) :
                skipcrop = 1
                current_time = datetime.datetime.now()
                tgl = str(current_time.year) + "-" + str(current_time.month) + "-" + str(current_time.day)
                jam = str(current_time.hour) + ":" + str(current_time.minute) + ":" + str(current_time.second)
            ...
            def show_frame():
                    ...
                    if frame is None:
                        ...
                        if ((cntdot % 4) == 0)  :
                            ...
                            #isExistDetectResult = os.path.exists(file)
                            #isExistResultCrop = os.path.exists(path_crop_img)
                            imgCrop = os.listdir(path_crop_img)
                            cek_img_crop = len(imgCrop)
                            if cek_img_crop > 0:
                                print("ada file crop")
                                #os.system("detect.py --weight=galian_200epch_1k5.pt --source=imagescrop/")
                        ...
                        isExistLastImg = os.path.exists('test_mse.jpg')
                        imageA = cv2.imread("test_mse.jpg")
                        imageB = frame
                        # convert the images to grayscale
                        imageA_GRY = cv2.cvtColor(imageA, cv2.COLOR_BGR2GRAY)
                        imageB_GRY = cv2.cvtColor(imageB, cv2.COLOR_BGR2GRAY)

                        print("Img1 Resolution:", imageA_GRY.shape)
                        print("Img2 Resolution:", imageB_GRY.shape)

                        cv2.imwrite('test_mse1.jpg', frame)
                        m = mse(imageA_GRY, imageB_GRY)
                        s = ssim(imageA_GRY, imageB_GRY)

                        label_m = f'{m:.2f}'
                        label_s = f'{s:.2f}'
                        print(label_m)
                        cv2.putText(current_frame_small,"MSE: " + str(label_m), (50, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,0), 2)
                        cv2.putText(current_frame_small,"SSIM: " + str(label_s), (50, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,0), 2)
                        #print("m: %.2f" + str(m))
...
btn_baseline = Button(fr_button, text="Upload Image",command=upload_image_func)
