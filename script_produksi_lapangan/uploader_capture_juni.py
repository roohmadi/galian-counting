# -*- coding: utf-8 -*-
"""
Created on Fri May 31 16:39:32 2024
...
global cntdot, tempCy, chtruk, skip_double0, chtruk0, get_diff_days
global filenameSave, filegambarstart,filenamestart
global cntOBJ, arah
...
class App:
    def __init__(self, window, window_title, video_source=val_rtsp):
        ...
        self.text_box = Text(fr_button,height=12,width=60)
        self.text_box.grid(row=21, column=1, sticky="ew", padx=10, pady=10)
        ...
        #--dummy
        #str_tgl = '2024_1_18'
        fileLog_cctv = path_log + str_tgl + '.log'
        self.read_log(fileLog_cctv)


        

        
        self.lbl_log = Label(fr_button, text="Log: ", font=('Times 14'))
        self.lbl_log.grid(row=21, column=0, padx=10, pady=5, sticky="w")
        ...
        self.btn_quit.grid(row=0, column=0, sticky="ew", padx=10, pady=10)
    ...
    def read_log (self, file):
        ...
        if os.path.isfile(file):
            #fileLog = path_log + fileLog            print("file Log open")
            print(file)
            f = open(file, "r")
            for message in f:
                self.text_box.insert(END, message)
