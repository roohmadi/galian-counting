...
class CCTVStream :
    ...
    def UploadIMGtoPedati(self,filenameSave, filename, muatan, source):
        ...
        if cctv_stream.connect():
			...
			data = {'id_muatan': muatan, 'pintu': PINTU, 'tanggal_capture': tgl_jam, 'filename': filenameSave, 'filegambar': filenameSave}
            ...
            if conf>0.5:
                ...
                if imageVal:
                    if(int(cls) == 1) or (int(cls) == 2):
                        ...
                        if (cy > YlineDetect) and (int(cls) == 1) and (skip_double == 0):
                            skip_double = 1
                            sabes_count += 1
