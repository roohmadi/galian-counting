...
global cntFr, lineDetect, cnf_last
cnf_last = 0
cntFr = 0
#----h-180
lineDetect = 250 #160 # 360-200
lineDetect1 =350 #240
        ...
        if (cntFr % 10) == 0:
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if webcam:  # batch_size >= 1
                    ...
                    im0 = cv2.resize(im,(0,0),fx=1,fy=1)
                ...
                if det is not None and len(det):
                    ...
                    if len(outputs) > 0:
                        for j, (output, conf) in enumerate(zip(outputs, confs)):
                            ...
                            #count
                            count_obj(bboxes,w,h,id,_dir,int(cls),im0,conf)
                            # print(im0.shape)
                ...
                if show_vid:
                    ...
                    cv2.line(im0, (0, lineDetect1), (w, lineDetect1), (255,0,0), thickness=2)
                    cv2.putText(im0,  "PINTU " + str(PINTU),(((w//2)-50),40),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
                ...
                #    1.5, (20,255,0), 3, cv2.LINE_AA)  
                    ...
                    try :
                        cv2.imshow('Penghitung Truk Muatan', im0)
...
def count_obj(box,w,h,id,direct,cls,im,conf):
    ...
    if direct=="South":
        cv2.putText(im,  "PINTU " + str(PINTU),(((w//2)-50),40),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
        #print("cy: " + str(cy) + " line: " + str(lineDetect) + " h: " + str(h) + " return: " + str(h//2))
        #print(tracker1)

        if ((cy >= lineDetect) and (cy <= lineDetect1)):
            if id not in tracker1:
                ...
                if (cls==1) and (conf>0.75):
                    ...
                    tgl = str(current_time.year) + "-" + str(current_time.month) + "-" + str(current_time.day)
                    jam = str(current_time.hour) + ":" + str(current_time.minute) + ":" + str(current_time.second)
                    filenameSave = "Img_" + str(current_time.year) + "_" + str(current_time.month) + "_" + str(current_time.day) +"_" + str(current_time.hour) + "_" + str(current_time.minute) + "_" + str(current_time.second) + "_1S.jpg"
                    ...
                    img_resize = cv2.resize(im,(0,0),fx=1,fy=1)
                    cv2.imwrite(filename, img_resize)
                elif (cls==2) and (conf>0.75):
                    truck_count+=1
                    print("Batu Belah")
                    ...
                    tgl = str(current_time.year) + "-" + str(current_time.month) + "-" + str(current_time.day)
                    jam = str(current_time.hour) + ":" + str(current_time.minute) + ":" + str(current_time.second)
                    filenameSave = "Img_" + str(current_time.year) + "_" + str(current_time.month) + "_" + str(current_time.day) +"_" + str(current_time.hour) + "_" + str(current_time.minute) + "_" + str(current_time.second) + "_2B.jpg"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default=val_rtsp, help='source')  # file/folder, 0 for webcam
